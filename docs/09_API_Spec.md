...
**음성 전용 모델:** `LLM_VOICE_MODEL`을 지정하면 음성 경로만 그 모델로 돈다
(미지정 시 `LLM_CHAT_MODEL`). 음성은 짧은 답이고 지연이 곧 체감 품질이라
더 빠른 모델이 유리하다 — 실측상 Haiku 4.5가 도구 선택에서 Sonnet 5보다
빠르고(1.5초 vs 3.6초) 정확도도 높았다.
