...
## 2026-07-27 (9) — "눌러도 반응 없음" 원인 규명·수정

사용자 보고를 재현하며 확인:

- **앱 자체는 정상**이었다. 자동화 클릭이 PointerEvent를 보내지 않아 react-native-web의
  `Pressable`이 반응하지 않은 것 — 실제 pointer 이벤트를 쏘니 정상 동작
- **다만 진짜 UX 결함이 있었다**: 권한 요청(getUserMedia)이 3초 이상 걸리는데
  그동안 화면이 `idle` 그대로여서 고장난 것처럼 보였다.
  → `preparing` 상태 추가 ("마이크를 여는 중이에요"), 누르는 즉시 전환
- **배너 문구가 틀렸다**: 마이크 권한 문제인데 "백엔드에 연결하지 못했습니다"로 표시.
  ConnectionNotice에 title을 받도록 하고 권한 안내 문구를 따로 지정
  (틀린 원인을 보여주면 사용자가 엉뚱한 곳을 고치게 된다)
- **Claude 인앱 브라우저 패널은 마이크가 차단**되어 있다 — 웹 테스트는 사용자의
  Chrome/Safari에서 해야 한다


## 2026-07-27 (8) — 음성 경로를 Haiku로 라우팅 (첫 소리 2.4~5.3초)

Haiku 4.5 vs Sonnet 5를 같은 도구 세트로 실측 비교:

| | 도구 선택 (5케이스) | 응답 작성 |
|---|---|---|
| Sonnet 5 | 평균 3.56초, **정확도 4/5** | 3.59초 (98tok), 문장 품질 우수 |
| Haiku 4.5 | 평균 **1.52초**, **정확도 5/5** | 1.33초 (74tok), 다소 밋밋 |

Sonnet이 오히려 일정 케이스에서 도구를 못 골랐다. 음성은 2~3문장이고 지연이
곧 체감 품질이라 **음성 경로만** `LLM_VOICE_MODEL`로 분리 (텍스트 채팅은 Sonnet 유지).

**발견한 잠재 결함:** 라운드 한도 폴백이 `tools`를 빼고 호출하는데,
히스토리에 `tool_calls`가 남아 있으면 **Bedrock이 502를 낸다**. 실측으로 확인하고
tools를 유지하도록 수정 (도구를 더 쓰지 말라는 건 말로 지시).
...
Haiku가 도구 호출 전에 "확인해드리겠습니다" 같은 선행 문장을 말할 때가 있는데,
그게 즉시 TTS로 나가면서 첫 소리가 2.5초까지 내려간다 — 사람이 말하는 방식과 같아
지연이 자연스럽게 가려진다. 다만 응답 길이 지침(2~3문장)을 Sonnet보다 덜 지켜
가끔 5문장이 나온다. 테스트 백엔드 48 + MCP 37.
